...
class TF_MultiHeadAttention(nn.Module):
    ...
    def forward(self, q, k, v, mask=None):
        ...
        if self.return_attention_scores:
            # attention_output: (bs, h, sl, value_dim), score: (bs, h, sl, sl)
            attention_output, score = attention(q, k, v, self.key_dim, mask, self.dropout,
                                                self.return_attention_scores)
        else:
            attention_output = attention(q, k, v, self.key_dim, mask, self.dropout,
                                         self.return_attention_scores)  # (bs, h, sl, value_dim)

        # concatenate heads and put through final linear layer
        # (bs, sl, heads * value_dim)
        concat = attention_output.transpose(1, 2).contiguous().view(bs, -1, self.h * self.value_dim)
...
class _TransformerLayer(nn.Module):
    ...
    def __init__(self,
        ...
        self.attn = TF_MultiHeadAttention(num_heads, feed_forward_size, layer_size, dropout=dropout_rate,
...
class Transformer(nn.Module):
    def __init__(self,
                 ...
                 return_attention_scores: bool = False,
                 max_seq_len: int = 256
                 # Maximum sequence length. This Transformer can't receive tokens that are more than this number.
                 ):
